...
//- PRIVATE & PROTECTED VARIABLES -------------------------------------------------------------------------
		...
		public static const LOOP_SUFFIX:String = '_CROSS_FADE';
...
//- PUBLIC & INTERNAL METHODS -----------------------------------------------------------------------------
		...
		public function removeSound($name:String):void
		{
			...
			if( si.crossFade )
				removeSound( si.name + LOOP_SUFFIX );
		...
		public function loopSound( name:String, volume:Number = 1 ):void
		{
			var si:SoundItem = (_soundsDict[ name ] as SoundItem);
		...
		public function createLoopCrossFade( name:String, introFadeLength:Number, endFadeLength:Number, volume:Number = 1 ):void
		{
			var si1:SoundItem = (_soundsDict[ name ] as SoundItem);
			if( !si1.crossFade )
			{
				si1.crossFade = true;
				addPreloadedSound( si1.sound, si1.name + LOOP_SUFFIX );
			}
			var si2:SoundItem = (_soundsDict[ name + LOOP_SUFFIX ] as SoundItem);
			if( si1.sound.length == 0 )
			{
				
			}
			loopCrossFade( si1, si2, introFadeLength, endFadeLength, volume );
		}
		
		public function loopCrossFade( si1:SoundItem, si2:SoundItem, introFadeLength:Number, endFadeLength:Number, volume:Number ):void
		{
			si1.play( 0, 0, 0 );

			si1.onFadeComplete.addOnce( function( si:SoundItem ):void
			{
				si1.onFadeComplete.addOnce( function( si:SoundItem ):void
				{
					startEndCrossFade( si1, si2, introFadeLength, endFadeLength, volume );
				});
				si1.fade( volume, ( si1.sound.length * 0.001 - introFadeLength - endFadeLength ) );
			});
			si1.fade( volume, introFadeLength, 0, false, Linear.easeOut );
		}
		
		public function startEndCrossFade( si1:SoundItem, si2:SoundItem, introFadeLength:Number, endFadeLength:Number, volume:Number ):void
		{
			si1.fade( 0, endFadeLength, 0, true, Linear.easeIn );
			loopCrossFade( si2, si1, introFadeLength, endFadeLength, volume );
		}
		 ...
		 * @return void
		 */
		public function stopAllSounds($useCurrentlyPlayingOnly:Boolean = false):void
		{
			var len:int = _sounds.length;
		 ...
		 * @return void
		 */
		public function fadeSound($name:String, $targVolume:Number = 0, $fadeLength:Number = 1, fadeDelay:Number = 0, $stopOnComplete:Boolean = false, removeListeners:Boolean = true ):void
		{
			var si:SoundItem = (_soundsDict[$name] as SoundItem);
			if( si.crossFade )
			{
		...
		public function setDistanceMultiplier( name:String, multi:Number ):void
		{
			var si:SoundItem = (_soundsDict[name] as SoundItem);
			if( si.crossFade )
			{
				var si2:SoundItem = (_soundsDict[name + LOOP_SUFFIX] as SoundItem);
				si2.distanceMultiplier = multi;
			}
			si.distanceMultiplier = multi;
		}
		
		public function panSound( name:String, pan:Number ):void
		{
			var si:SoundItem = (_soundsDict[name] as SoundItem);
			si.pan( pan );
			if( si.crossFade )
